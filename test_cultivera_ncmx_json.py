#!/usr/bin/env python3
"""
Test Cultivera JSON Matching with NCMX, LLC Data
Tests JSON matching functionality with the provided NCMX Cultivera manifest.
"""
...
def test_ncmx_json_download():
    """Test downloading and parsing the NCMX Cultivera JSON file."""
    print("=== Testing NCMX Cultivera JSON Download and Parsing ===")
    
    url = "https://files.cultivera.com/435553542D57533130383735/Interop/25/29/H0PWWVA0R4EQ8TH7/Cultivera_ORD-21658_422044.json"
...
def test_ncmx_json_structure(data):
    """Test the structure of the NCMX Cultivera JSON data."""
    print("\n=== Testing NCMX Cultivera JSON Structure ===")
...
def test_ncmx_product_extraction(data):
    """Test product extraction from NCMX data."""
    print("\n=== Testing NCMX Product Extraction ===")
...
def test_ncmx_specific_fields(data):
    """Test NCMX-specific field handling."""
    print("\n=== Testing NCMX-Specific Fields ===")
...
def test_ncmx_product_validation(data):
    """Test product validation and data quality for NCMX data."""
    print("\n=== Testing NCMX Product Validation ===")
    ...
    for item in items:
        ...
        # Check for edible products
        if item.get('inventory_type') == 'Solid Edible':
            edible_products += 1
    ...
    print(f"✅ Edible products: {edible_products}/{len(items)}")
    
    # Show sample products
    print("\nSample products from NCMX manifest:")
    for i, item in enumerate(items[:5]):
        print(f"  {i+1}. {item.get('product_name', 'Unknown')} - {item.get('strain_name', 'No strain')}")
        print(f"     Type: {item.get('inventory_type', 'Unknown')}")
    
    return True

def test_ncmx_manifest_processing(data):
    """Test full manifest processing for NCMX data."""
    print("\n=== Testing NCMX Manifest Processing ===")
    ...
    print("NCMX Manifest Information:")
...
def test_ncmx_edible_specific_processing(data):
    """Test edible-specific processing for NCMX data."""
    print("\n=== Testing NCMX Edible-Specific Processing ===")
    
    items = data.get('inventory_transfer_items', [])
    edible_items = [item for item in items if item.get('inventory_type') == 'Solid Edible']
    
    print(f"✅ Found {len(edible_items)} edible products")
    
    if edible_items:
        print("\nEdible product analysis:")
        for i, item in enumerate(edible_items[:3]):
            print(f"  {i+1}. {item.get('product_name', 'Unknown')}")
            print(f"     Strain: {item.get('strain_name', 'No strain')}")
            print(f"     Weight: {item.get('unit_weight', 0)} {item.get('unit_weight_uom', '')}")
            print(f"     Price: ${item.get('line_price', 0)}")
            
            # Check for cannabinoid ratios in product names
            product_name = item.get('product_name', '')
            if ':' in product_name:
                print(f"     Contains cannabinoid ratio: {product_name}")
            ...
            if lab_data:
                potency = lab_data.get('potency', [])
                for p in potency:
                    if p.get('type') in ['thc', 'cbd', 'cbg', 'cbn']:
                        print(f"     {p.get('type').upper()}: {p.get('value', 0)} {p.get('unit', '')}")
    
    return True

def test_json_matcher_compatibility():
    """Test JSON matcher compatibility with NCMX data."""
    print("\n=== Testing JSON Matcher Compatibility ===")
    
    try:
        from src.core.data.json_matcher import JSONMatcher, map_json_to_db_fields
        
        # Test field mapping function with NCMX-style data
        test_item = {
            'product_name': 'DOHC - Huckleberry Gummies - Hybrid Enhanced - 100mg THC',
            'strain_name': 'Hybrid Enhanced',
            'line_price': 150.0,
            'unit_weight': 40.0,
            'inventory_type': 'Solid Edible',
            'inventory_category': 'EndProduct'
        }
        
        mapped = map_json_to_db_fields(test_item)
        print(f"✅ Field mapping works with NCMX data: {mapped}")
        
        return True
        
    except Exception as e:
        print(f"❌ JSON matcher compatibility test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    """Main test function."""
    print("🧪 NCMX Cultivera JSON Matching Test")
    print("=" * 50)
    
    # Test 1: Download JSON
    data = test_ncmx_json_download()
    if not data:
        print("❌ Cannot proceed without JSON data")
        return False
    
    # Test 2: Validate structure
    if not test_ncmx_json_structure(data):
        print("❌ JSON structure validation failed")
        return False
    
    # Test 3: Test product extraction
    products = test_ncmx_product_extraction(data)
    if not products:
        print("❌ Product extraction failed")
        return False
    
    # Test 4: Test NCMX-specific fields
    if not test_ncmx_specific_fields(data):
        print("❌ NCMX-specific field test failed")
        return False
    
    # Test 5: Test product validation
    if not test_ncmx_product_validation(data):
        print("❌ Product validation test failed")
        return False
    
    # Test 6: Test manifest processing
    if not test_ncmx_manifest_processing(data):
        print("❌ Manifest processing test failed")
        return False
    
    # Test 7: Test edible-specific processing
    if not test_ncmx_edible_specific_processing(data):
        print("❌ Edible-specific processing test failed")
        return False
    
    # Test 8: Test JSON matcher compatibility
    if not test_json_matcher_compatibility():
        print("❌ JSON matcher compatibility test failed")
        return False
    
    print("\n" + "=" * 50)
    print("🎉 ALL TESTS PASSED! JSON matching works with NCMX data!")
    print("=" * 50)
    print(f"📊 Processed {len(products)} products from NCMX manifest")
    print("✅ JSON matching system is compatible with different vendors")
    print("✅ Edible product processing verified")
    print("=" * 50)
