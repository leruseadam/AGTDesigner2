#!/usr/bin/env python3
"""
Test script to verify session persistence between requests.
"""
...
def test_session_persistence():
    ...
    # Step 3: Perform JSON matching with the same session
    print("\n3. Performing JSON matching with session...")
    ...
    try:
        response = session.post(f'{base_url}/api/json-match', 
                               json={'url': json_url},
    ...
    # Step 4: Check selected tags immediately after JSON match (same session)
    print("\n4. Checking selected tags immediately after JSON match (same session)...")
    try:
